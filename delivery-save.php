	...
	case 'zip':
		...
		if (!empty($zipcode)) {
			...
			if (empty($location['zip'])) { // Try our local file
				error_log(sprintf("%s line: %s: ZIP {$zipcode} not found via Nominatim, trying local CSV", __FILE__, __LINE__));
...
function hc_get_store_data($field, $zone) {
	...
	if (!$post || !isset($post->ID)) {
		error_log(sprintf("%s line: %s: hc_get_store_data: could not find 'our-stores/" . $zone, __FILE__, __LINE__));
